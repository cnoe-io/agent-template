# 🚀 Pet Store AI### 1️⃣ Environment Setup
...
# Run the A2A server
poetry run python -m agent_petstore --host localhost --port 8000
```n](https://img.shields.io/badge/python-3.13%2B-blue?logo=python)](https://www.python.org/)
[![Poetry](https://img.shields.io/badge/poetry-2.1.1%2B-blueviolet?logo=python)](https://python-poetry.org/)
[![License](https://img.shields.io/badge/license-Apache%202.0-green)](LICENSE)

---

- 🤖 **Pet Store Agent** is an LLM-powered agent built using the [LangGraph ReAct Agent](https://langchain-ai.github.io/langgraph/agents/agents/) workflow and Pet Store [MCP Server](https://modelcontextprotocol.io/introduction).
...
- 🔌 **MCP Tools:** Uses [langchain-mcp-adapters](https://github.com/langchain-ai/langchain-mcp-adapters) to glue the tools from Pet Store MCP server to LangGraph ReAct Agent Graph.
...
flowchart TD
  ...
  subgraph Tools Layer
    ...
    E[Pet Store MCP Server]
    F[Pet Store API Server]
  end
...
sequenceDiagram
  ...
  participant LLM
  participant MCP as Pet Store MCP Server
  participant APIServer as Pet Store API Server
...
Use this setup to test the agent with the Pet Store API.
...
## 🐾 Pet Store API Capabilities

This agent integrates with the Pet Store API, providing the following capabilities:
...
### 🛒 Store Operations
- Get inventory status by pet status
...
### 👤 User Operations
- Create new users
- Create users with lists or arrays
- Login and logout users
- Get user details by username
- Update user information
- Delete users

The agent uses these capabilities to help users interact with the Pet Store API through natural language, handling various operations and retrieving information as needed.
